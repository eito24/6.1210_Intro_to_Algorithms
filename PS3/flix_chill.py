...
class Movie:
    ...
    def __init__(self, id, date, rank):
        ...
        ##MY CODE HERE
        self.hnode = self.key #stores key of highest ranking movie on or after self.key
        self.hrank = self.rank
        self.hid = self.id
...
class FlixChillManager(AVL):
    ...
    def maintain(self):
        """"""
        t_date = self.item.key
        t_hrank = self.item.hrank
        t_hnode = self.item.hnode
        t_hid = self.item.hid
        r_hnode  = self.right.item.hnode  if self.right  else t_hnode
        r_hrank = self.right.item.hrank  if self.right  else t_hrank
        r_hid = self.right.item.hid if self.right else t_hid
        leftdate = self.left.item.key if self.left else  None
        if leftdate is not None:
            if leftdate-t_date == 0: #if they start on the same date can be considered for highest rank
                l_hnode  = self.left.item.hnode
                l_hrank = self.left.item.hrank
                l_hid = self.left.item.hid
                if l_hrank > t_hrank & l_hrank > r_hrank:
                    self.item.hrank = l_hrank
                    self.item.hnode = l_hnode
                    self.item.hid = l_hid
        if r_hrank > t_hrank:
            self.item.hrank = r_hrank
            self.item.hnode = r_hnode
            self.item.hid = r_hid
        if self.parent:
            self.parent.maintain()      
    ...
    def highest(self, s):
        ...
        """
        ## MY CODE HERE
        earliest_node = self.earliest(s) #movie that can be streamed earliest on or after s
        return earliest_node.item.hid
